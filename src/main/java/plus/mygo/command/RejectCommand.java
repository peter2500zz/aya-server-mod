 ...
 * /reject 指令。
 * <p>
 * 仅限玩家执行。拒绝指定玩家发给自己的传送请求，可选附带一句原因转达给对方。
 * <p>
 * 请求之间互不排斥（见 {@link TpaRequests}），同一时刻可能有多人在等自己回应，
 * 因此必须指明拒绝的是谁。玩家名的补全由 {@link TpaRequests#suggestIncoming} 提供，
 * <b>只列出此刻正在等待自己回应的发起者</b>，而非全体在线玩家。
 * <p>
 * 用法：{@code /reject <玩家>} 或 {@code /reject <玩家> <原因>}。
 * 也可以点击请求提示中的【拒绝】按钮，该按钮填入的指令<b>末尾带一个空格</b>，
 * 使光标落在原因参数位上，从而借原版的命令提示告知玩家这里还可以写原因。
 */
public class RejectCommand {

    /** 玩家参数名，用于从 CommandContext 中按名称读取参数值。 */
    ...
    private static int execute(CommandContext<CommandSourceStack> context, String reason)
        ...
        // 拒绝成功时的双方通知由 TpaRequests 发出，此处只需处理「无此请求」的情况
        if (TpaRequests.reject(target, requester, reason) == 0) {
